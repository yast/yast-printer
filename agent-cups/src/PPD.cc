...
string PPD::clean(const char *s) {
    if(s) return killbraces(string(s));
    else return string("");
}
...
void PPD::preprocess(PPD::PPDInfo info, PPDInfo *newinfo) {
    ...
    /*
    if(((signed)printer.find("CUPS")!=-1) || ((signed)printer.find("ESP")!=-1) || printer=="")
        printer = killbraces(info.product);
    */

    /* old....
    if((signed)printer.find("CUPS")!=-1)
        printer=ppd->modelname?:"";
    if((signed)printer.find("ESP")!=-1) {
        printer=ppd->modelname?:"";
        //printer.erase(0,FFO(printer," ")+1); // fixme: +1
    }
    */
